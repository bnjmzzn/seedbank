...
const validBody = { fromUsername: "user2", amount: STEAL_MIN };
...
describe("POST /api/steal", () => {
    ...
    it("returns 400 if fromUsername is missing", async () => {
        const res = await POST(makeRequest({ amount: STEAL_MIN }, "user-123"));
        const body = await res.json();
        expect(res.status).toBe(400);
        expect(body.data).toHaveProperty("fromUsername");
    });
    ...
    it("returns 400 if amount is below minimum", async () => {
        const res = await POST(makeRequest({ fromUsername: "user2", amount: STEAL_MIN - 1 }, "user-123"));
        const body = await res.json();
        expect(res.status).toBe(400);
        expect(body.data).toHaveProperty("amount");
    });
    ...
    it("returns 400 if amount is above maximum", async () => {
        const res = await POST(makeRequest({ fromUsername: "user2", amount: STEAL_MAX + 1 }, "user-123"));
        const body = await res.json();
        expect(res.status).toBe(400);
        expect(body.data).toHaveProperty("amount");
    });
    ...
    it("returns 200 on success", async () => {
        vi.mocked(stealBalance).mockResolvedValueOnce({ success: true, delta: STEAL_MIN, balance: 900 });
    ...
    it("returns 400 if steal limit exceeded", async () => {
