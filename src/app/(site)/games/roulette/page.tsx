...
import RouletteGameBox from "./_components/RouletteGameBox";

import { HistoryReason } from "@/types/models";
import type { GameResult } from "@/types/api";

const game = GAMES.find((game) => game.id === HistoryReason.Game.ROULETTE)!;
        ...
        <Stack gap={4} sx={{ minWidth: 0, overflow: "hidden", p: { sm: 1, md: 2 } }}>
            <Stack direction="row" flexWrap="wrap">
                ...
                <Stack flex={1} gap={2}>
                    ...
                    />
                    <GameStatusDisplay isLocked={isLocked} result={displayResult} amountValid={amountValid} />
                </Stack>
            </Stack>
            <Stack direction="row" flexWrap="wrap" gap={4}>
                <Stack flex={2} gap={1} minWidth={300}>
                    <SectionHeader icon="mdi:history" label="Recent Activity" />
                    <HistoryTable rows={rows} type={game.id} isLoading={historyLoading} maxRowsPerPage={5} />
                </Stack>
